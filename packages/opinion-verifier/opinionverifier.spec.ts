...
describe("OpinionVerifier tests", () => {
  ...
  it("should change hash value to sha256('hello world')", async () =>  {
    await opinionVerifierContract.sendIncrement(wallet1.getSender(), "hello world");
  ...
  it("should change hash value to sha256('another string')", async () =>  {
    await opinionVerifierContract.sendIncrement(wallet1.getSender(), "another string");
    const opinionVerifierValue = await opinionVerifierContract.getInfo();
  ...
  it("should change hash value twice", async () =>  {
    await opinionVerifierContract.sendIncrement(wallet1.getSender(), "string number 1");
    await opinionVerifierContract.sendIncrement(wallet1.getSender(), "string number 2");
    const opinionVerifierValue = await opinionVerifierContract.getInfo();
  ...
  it("should send ton coin to the opinionVerifier", async () => {
    ...
    const value = await opinionVerifierContract.getBalance();
    console.log(value)
